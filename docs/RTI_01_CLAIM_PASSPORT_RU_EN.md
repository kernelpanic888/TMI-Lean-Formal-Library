...
- **Status:** Lean-kernel-checked core + independent first-order ATP mirror; physical bridge open.
...
### 4.1. Независимое ATP-зеркало

Только теорема `future_reading_cannot_be_both` независимо переведена в first-order TPTP-задачу:
...
Аудит на commit `17058ba82066732733fec34ea5f305f380495482` получил точный статус `SZS status Theorem` от **Vampire 5.0.1** и **E 3.2.5**. Это независимое логическое зеркало непересечения, а не замена Lean kernel и не доказательство физического разворота времени. Зависимые типы `TimeTouch`, `ReunionExperiment` и конечная модель остаются в области Lean-аудита.
...
- квантовая суперпозиция мировых линий, часов или отображений сравнения;
...
4. Задать квантовый объект: суперпозицию путей, часов, касаний или самих сравнений.
...
### 4.1. Independent ATP mirror

Only `future_reading_cannot_be_both` is mirrored as a first-order TPTP problem. At commit `17058ba82066732733fec34ea5f305f380495482`, **Vampire 5.0.1** and **E 3.2.5** both returned the exact result `SZS status Theorem`. This independently mirrors the disjointness argument; it neither replaces the Lean kernel nor proves a physical reversal of time. Dependent `TimeTouch` and `ReunionExperiment` structures and the finite model remain Lean-side certificates.

### 5. Red boundary

RTI-01 currently proves no physical reversal in an ordinary time-oriented GR spacetime, no motion into one’s own causal past, no closed timelike curve, no physical realization of \(\sigma=-1\), no quantization, and no empirical quantum time machine. Gravitational time dilation and differential aging alone do not determine \(\sigma(p,q)\).
...
4. Define the quantum object: a superposition of routes, clocks, touches, or comparison maps.
...
- First-order mirror: `external_proofs/rti_01_future_reading_cannot_be_both_tptp_0_1.p`
