...
Справочная формулировка:

```text
Ruliad :=
  limiting mathematical object
  representing a superposition of possible computational processes
  generated by possible rules
```
...
```text
рулиадная цепочка является отдельной канонической осью записи,
параллельной матрице (Я)-Я-я, но не заменяющей её
```
...
## 2.3. Доменный Сдвиг И Искривление

Манипуляция вокруг гипотетической оси сама по себе не является физическим
утверждением. Но если в этой манипуляции появляется высший тик и он выходит из
синхрона с проекционным срезом, модель читает это как доменный сдвиг:

```text
HigherTick(τ)
ProjectionDesync(τ, s)
HypotheticalRotationManipulation(ω, τ)
  -> DomainShift(ω, τ, s)
```

Доменный сдвиг порождает искривление проекционной поверхности:
...
```text
ProjectionCurvature is GR-inspired curvature reading
ProjectionCurvature != proof of Einstein equations
ProjectionCurvature requires guard before empirical claim
```

По-русски:

```text
высший тик рассинхронизируется с проекциями;
возникает доменный сдвиг;
доска читает этот сдвиг как искривление поверхности
```
...
```text
TickOfContact is not a literal identity of time with memory
TickOfContact is not a literal identity of time with light speed
