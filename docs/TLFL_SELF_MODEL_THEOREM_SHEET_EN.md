...
| 7 | classified `TLFL + External Proof Layer {Z3, Vampire, E}` proof state | organize proof-state maps | proof-state self-model | proof-state self-model does not imply empirical truth | `tmi_lean_formal_library_0_1/lean/TMI/ProofChainSelfModel.lean` | module-level `TLFL + External Proof Layer {Z3, Vampire, E}` proof-state self-model | `tlfl_proof_self_model_z3_0_1.smt2`, `tlfl_proof_self_model_tptp_0_1.p` | proof-state self-model available |
