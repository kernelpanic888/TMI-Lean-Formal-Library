...
| 7 | classified `TLFL + External Proof Layer {Z3, Vampire, E}` proof state | организовать proof-state maps | proof-state self-model | proof-state self-model не дает empirical truth | `tmi_lean_formal_library_0_1/lean/TMI/ProofChainSelfModel.lean` | module-level самомодель proof-state для `TLFL + External Proof Layer {Z3, Vampire, E}` | `tlfl_proof_self_model_z3_0_1.smt2`, `tlfl_self_model_proof_tptp_0_1.p` | модель доказательного состояния доступна |
